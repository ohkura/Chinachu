...
exports.isMatchedProgram = function (rules, program, nf) {
	...
	for (i = 0; i < rules.length; i++) {
		matched_program = exports.programMatchesRule(rules[i], program, nf, fullTitle_norm, detail_norm, matched_program);
	}
...
// 単体のルールとのマッチ判定
exports.programMatchesRule = function (rule, program, nf, fullTitle_norm, detail_norm, matched_program) {
	var i, j, l, m, isFound;
	...
	// isDisabled
	if (rule.isDisabled) { return matched_program; }

	// sid
	if (rule.sid && rule.sid !== program.channel.sid) { return matched_program; }

	// types
	if (rule.types) {
		if (rule.types.indexOf(program.channel.type) === -1) { return matched_program; }
	}
	...
	// category
	if (rule.category && rule.category !== program.category) { return matched_program; }

	// categories
	if (rule.categories) {
		if (rule.categories.indexOf(program.category) === -1) { return matched_program; }
	}
	...
	if (rule.hour && (typeof rule.hour.start === 'number') && (typeof rule.hour.end === 'number') && !(rule.hour.start === 0 && rule.hour.end === 24)) {
		...
		if (ruleStart > ruleEnd) {
			if ((ruleStart > progStart) && (ruleEnd < progEnd)) { return matched_program; }
		} else {
			if ((ruleStart > progStart) || (ruleEnd < progEnd)) { return matched_program; }
		}
	}
	...
	if (rule.duration && (typeof rule.duration.min !== 'undefined') && (typeof rule.duration.max !== 'undefined')) {
		if ((rule.duration.min > program.seconds) || (rule.duration.max < program.seconds)) { return matched_program; }
	}
	...
	if (rule.reserve_titles) {
		...
		if (!isFound) { return matched_program; }
	}
	...
	if (rule.ignore_titles) {
		for (i = 0; i < rule.ignore_titles.length; i++) {
			if (nf) {
				if (fullTitle_norm.match(rule.ignore_titles[i].normalize(nf)) !== null) { return matched_program; }
			}
			else {
				if (program.fullTitle.match(rule.ignore_titles[i]) !== null) { return matched_program; }
			}
		}
	}
	...
	if (rule.reserve_descriptions) {
		if (!program.detail) { return matched_program; }
	...
	if (rule.ignore_descriptions) {
		...
		for (i = 0; i < rule.ignore_descriptions.length; i++) {
			if (nf) {
				if (detail_norm.match(rule.ignore_descriptions[i].normalize(nf)) !== null) { return matched_program; }
			}
			else {
				if (program.detail.match(rule.ignore_descriptions[i]) !== null) { return matched_program; }
			}
		}
	}
	...
	if (rule.ignore_flags) {
		for (i = 0; i < rule.ignore_flags.length; i++) {
			for (j = 0; j < program.flags.length; j++) {
				if (rule.ignore_flags[i] === program.flags[j]) { return matched_program; }
			}
		}
	}
	...
	if (rule.reserve_flags) {
		if (!program.flags) { return matched_program; }
	    ...
	    matched_program.keep_days = rule.keep_days;
	    matched_program.convert_mp4 = rule.convert_mp4;
	} else {
	    if (rule.keep_days == 0) {
		matched_program.keep_days = 0;
	    } else if (matched_program.keep_days < rule.keep_days) {
		matched_program.keep_days = rule.keep_days;
	    }
	    matched_program.convert_mp4 = matched_program.convert_mp4 || rule.convert_mp4;
	}
        return matched_program;
};
